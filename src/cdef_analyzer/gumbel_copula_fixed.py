...
from scipy.stats import chi2_contingency, entropy, kendalltau
...
class RankDependencyAnalyzer:
    ...
    def summarize_pairwise_mi_chi2(self, rankings_df: pd.DataFrame) -> Tuple[float, float, float]:
        """
        Summarize dependence across all rater pairs.
        Returns (avg_MI, min_p_value, max_chi2) over all pairs.
        """
        cols = list(rankings_df.columns)
        mis, pvals, chi2s = [], [], []
        for i, c1 in enumerate(cols):
            for c2 in cols[i + 1:]:
                mi, p, chi2 = self.compute_mutual_information_and_independence(
                    rankings_df[c1].values, rankings_df[c2].values
                )
                mis.append(mi)
                pvals.append(p)
                chi2s.append(chi2)
        ...
        avg_mi = float(np.round(np.mean(mis), 3))
        min_p = float(np.round(np.min(pvals), 3))
        max_chi2 = float(np.round(np.max(chi2s), 3))
        return avg_mi, min_p, max_chi2
    ...
    def estimate_copula_theta(self, rankings_df: pd.DataFrame) -> float:
        ...
        self.theta = float(np.round(theta_gumbel * (1 + self.W), 3))
        return self.theta

    # --------------------------
    # Copula fitting & likelihood
    # --------------------------
        ...
        for i, r1 in enumerate(raters):
            for r2 in raters[i + 1:]:
                try:
                    ...
                    # Transform to uniform [0,1] using empirical CDF (mid-ranks)
    ...
    def compute_avg_log_likelihood(self, rankings_df: pd.DataFrame) -> float:
        ...
        return float(np.round(np.mean(log_likelihoods), 6))
    ...
    def fit_mallows_model(self, rankings_df: pd.DataFrame) -> Dict:
        ...
        # Estimate theta (simple MLE-style approximation)
        # For Mallows: E[d] ≈ n(n-1)/(4 * (1 + exp(θ)))
        # Rough inverse: theta ≈ log((n(n-1))/(4*avg_distance) - 1)
        if avg_distance > 0:
            theta_mallows = max(0.01, math.log(max(n * (n - 1) / (4 * avg_distance) - 1, 1.01)))
        else:
            theta_mallows = 10.0  # Perfect agreement

        # Approximate log-likelihood (partition function omitted)
        log_likelihood = -theta_mallows * sum(distances)

        return {
            'theta': float(np.round(theta_mallows, 3)),
            'consensus': consensus,
            'avg_distance': float(np.round(avg_distance, 2)),
            'log_likelihood': float(np.round(log_likelihood, 3))
        }

    def calculate_multinomial_log_likelihood(self, rankings_df: pd.DataFrame) -> float:
        """
        Compute log-likelihood under a simple multinomial baseline for non-forced independent data.
        ...
        # Count frequencies
        _, counts = np.unique(all_pairs, axis=0, return_counts=True)
        ...
        log_likelihood = np.sum(counts * np.log(probs + 1e-12))
        return float(np.round(log_likelihood, 3))

    def choose_distribution_model(
        self, rankings_df: pd.DataFrame, is_dependent: Optional[bool] = None
        ...
        if is_dependent is None:
            # Fallback: test a single pair if caller didn't supply a decision
        ...
        if ranking_type == 'forced':
            if is_dependent:
                ...
                return 'Uniform Permutation (forced, independent)', float(np.round(log_likelihood, 3))
        else:  # non-forced
            if is_dependent:
                # Just report that dependence detected, rely on copula layer
    ...
    def compute_relative_importance(self, W: float, mi: float, theta: float) -> Dict[str, float]:
        ...
        return {
            'Concordance': float(np.round(W / total, 3)),
            'Concurrence': float(np.round(mi / total, 3)),
            'Extremeness': float(np.round(theta / total, 3))
        }
    ...
    def analyze_from_excel(
        ...
        # Pairwise dependence summary across ALL rater pairs
        mi, p_value, chi2_stat = self.summarize_pairwise_mi_chi2(rankings_wide)
        is_dependent = (p_value < self.significance_level)

        # Distribution model selection (use global dependence decision)
        distribution_model, model_log_likelihood = self.choose_distribution_model(
            rankings_wide, is_dependent=is_dependent
        )
        ...
        pairwise_thetas = {}
        for pair_name, copula in self.copulas.items():
            pairwise_thetas[pair_name] = float(np.round(copula.theta, 3))
        ...
        for c1 in rankings_wide.columns:
            for c2 in rankings_wide.columns:
                if c1 == c2:
                    ...
                    tau, _ = kendalltau(rankings_wide[c1], rankings_wide[c2])
                    tau_matrix.loc[c1, c2] = float(np.round(tau, 3))

        return CopulaResults(
            theta_scaled=theta_scaled,
            theta_gumbel=theta_gumbel,
            kendalls_W=self.W,
            avg_kendalls_tau=float(np.round(avg_tau, 3)),
...
def format_results(results: CopulaResults) -> str:
    ...
    output.append(f"\nDependence Tests (pairwise summary):")
    output.append(f"  Mutual information (nats):     {results.mutual_information}")
    output.append(f"  Max chi-square statistic:      {results.chi_square_stat}")
    output.append(f"  Min p-value across pairs:      {results.p_value}")
